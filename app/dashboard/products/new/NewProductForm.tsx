...
export function NewProductForm({ remaining }: NewProductFormProps) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (result.code === "PLAN_LIMIT_EXCEEDED") {
          throw new Error("プラン上限に達しています。アップグレードしてください。");
        }
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">新しいプロダクトを追加</h1>
        <p className="text-gray-600 mt-1">SaaSやサービスの情報を入力して出品しましょう</p>
        {remaining !== null && (
        ...
        <div className="card p-6 space-y-6">
          <div>
            ...
            <div className="space-y-4">
              ...
              <FormField id="name" name="name" label="プロダクト名" required placeholder="例: クラウド請求書" error={fieldErrors.name} />
              <FormField id="tagline" name="tagline" label="キャッチコピー" required placeholder="例: 請求書作成から入金管理まで、すべてをクラウドで完結" maxLength={100} hint="100文字以内で簡潔に" validate={validateTagline} error={fieldErrors.tagline} />
              <TextareaField id="description" name="description" label="詳細説明" required rows={8} placeholder="プロダクトの特徴、機能、利用シーンなどを詳しく記載してください。Markdown形式で記述できます。" error={fieldErrors.description} />
              <SelectField id="category" name="category" label="カテゴリー" required placeholder="選択してください" options={categories.map((c) => ({ value: c.id, label: c.name }))} error={fieldErrors.category} />
            </div>
          </div>

          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-4">料金体系</h2>
            <div className="space-y-4">
              <div>
                <label className="label">料金タイプ <span className="text-red-500">*</span></label>
                <div
                ...
                >
                  {pricingTypes.map((type) => (
                    <label key={type.id} className="flex items-start gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50 transition-colors">
                      <input type="radio" name="pricing_type" value={type.id} className="mt-1" />
                      <div>
                ...
                {fieldErrors.pricing_type && (
                  <p className="text-sm text-red-600 mt-1" role="alert">{fieldErrors.pricing_type}</p>
                )}
              </div>
              <FormField id="price_info" name="price_info" label="料金詳細" placeholder="例: ¥980/月〜、¥500/ユーザー/月" hint="具体的な料金がある場合は記載してください" />
            </div>
          </div>

          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-4">スクリーンショット</h2>
            <p className="text-sm text-gray-600 mb-4">プロダクトの画面イメージを最大5枚までアップロードできます</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              {screenshots.length < 5 && (
                <ImageUpload currentImage={null} onUpload={(url) => setScreenshots([...screenshots, url])} type="screenshot" size="lg" shape="square" />
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <FormField id="website_url" name="website_url" type="url" label="公式サイトURL" placeholder="https://example.com" validate={validateUrl} error={fieldErrors.website_url} />
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50">
                <input type="radio" name="is_published" value="true" defaultChecked />
                <div>
                  <p className="font-medium text-gray-900">公開する</p>
                  <p className="text-xs text-gray-500">すぐにマーケットプレイスに掲載されます</p>
                </div>
              </label>
              <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">あとから公開できます</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-gray-200">
            ...
            <Link href="/dashboard/products" className="btn btn-secondary">キャンセル</Link>
          </div>
        </div>
      </form>
    </div>
